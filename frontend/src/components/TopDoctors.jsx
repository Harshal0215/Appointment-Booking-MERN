...
const {doctors} = useContext(AppContext)
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
        ...
        <p className='sm:w-1/3 text-center text-sm'>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Soluta, eveniet.</p>
        <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 sm:px-0'>
            {doctors.slice(0,10).map((item,index)=>(
                <div onClick={()=>navigate(`/my-appointments/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:scale-[104%] hover:translate-y-[-10px] transition-all duration-500' key={index}>
